...
class TerminalEmulator {
  ...
  processCommand(command) {
    ...
    if (this.commandList[cmd]) {
      ...
      this.addResponse(
        `Comando não encontrado: ${cmd}. Digite 'help' para ver os comandos disponíveis.`
      );
    }
  }
...
[HackTheBox] https://app.hackthebox.com/
  ...
  catFile(args) {
    const files = {
      "about.txt":
        "Gabriel - Especialista em Segurança da Informação\nFoco em pentesting e bug bounty.\n\nPara mais informações, use o comando 'about'.",
      "skills.dat":
        "Arquivo de habilidades. Use o comando 'skills' para visualizar o conteúdo formatado.",
      "mission.txt": "Protegendo o futuro, quebrando barreiras no presente.",
      "projects.log":
        "Registro de projetos. Use o comando 'projects' para detalhes.",
      "contacts.enc":
        "Arquivo criptografado. Use o comando 'contact' para visualizar informações de contato.",
      "certifications.md":
...
/**
 * DASHBOARD.JS - SECURITY DASHBOARD PARA PORTFÓLIO
 * Implementação de dashboard interativo para portfólio de especialista em segurança
 */

// Classe que gerencia o Security Dashboard
class SecurityDashboard {
  constructor(selector) {
    this.dashboard = document.querySelector(selector);
    if (!this.dashboard) return;

    // Inicia a animação das métricas após um pequeno delay
    setTimeout(() => {
      this.animateMetrics();
    }, 1000);

    // Adiciona animação para os elementos do dashboard
    this.setupDashboardAnimations();

    // Inicializa os eventos interativos
    this.setupInteractivity();
  }

  // Anima as barras de métricas
  animateMetrics() {
    const metrics = this.dashboard.querySelectorAll(".radar-metric");
    metrics.forEach((metric, index) => {
      // Delay sequencial para cada métrica
      setTimeout(() => {
        // Força um reflow para que a animação seja aplicada
        metric.style.width = metric.style.width;
      }, index * 300);
    });
  }

  // Configuração das animações visuais
  setupDashboardAnimations() {
    // Efeito de entrada para cards
    this.animateElement(".dashboard-card", "fadeInUp", 300);

    // Adiciona efeito hover para cards
    const cards = this.dashboard.querySelectorAll(".dashboard-card");
    cards.forEach((card) => {
      card.addEventListener("mouseenter", () => {
        card.style.transform = "translateY(-5px)";
        card.style.boxShadow = "0 8px 25px rgba(0, 150, 255, 0.4)";
      });

      card.addEventListener("mouseleave", () => {
        card.style.transform = "translateY(0)";
        card.style.boxShadow = "0 5px 15px rgba(0, 150, 255, 0.25)";
      });
    });

    // Animação para as tags
    const tags = this.dashboard.querySelectorAll(".tag");
  ...
  // Configuração da interatividade do dashboard
  setupInteractivity() {
    // Adiciona evento para o indicador de status
    const statusIndicator = this.dashboard.querySelector(".status-indicator");
    const statusText = this.dashboard.querySelector(".status-text");

    if (statusIndicator && statusText) {
      // Simula mudanças de estado do sistema
      setInterval(() => {
        // Simula uma pequena verificação de segurança
        this.simulateSecurityCheck(statusIndicator, statusText);
      }, 8000);
    }

    // Adiciona tooltips para métricas quando hover
    const metricBars = this.dashboard.querySelectorAll(".metric-bar");
    metricBars.forEach((bar) => {
      bar.addEventListener("mouseenter", () => {
        const value = bar.querySelector(".metric-value").textContent;
        const label =
          bar.parentElement.querySelector(".metric-label").textContent;
        this.showTooltip(bar, `${label}: ${value}`);
      });
  ...
  // Simula uma verificação de segurança com mudança visual de status
  simulateSecurityCheck(indicator, text) {
    // Mudança visual para indicar verificação em andamento
    indicator.classList.remove("online");
    indicator.classList.add("checking");
    text.textContent = "Verificando...";
    text.style.color = "#f39c12";

    // Após um tempo, volta ao status seguro
    setTimeout(() => {
  ...
  showTooltip(element, text) {
    ...
    // Posiciona e adiciona o tooltip ao documento
  ...
  // Anima entrada de elementos com delay
  animateElement(selector, animationClass, delay) {
    const elements = this.dashboard.querySelectorAll(selector);
...
// Inicializa o dashboard quando a página for carregada
document.addEventListener("DOMContentLoaded", () => {
  const dashboard = new SecurityDashboard(".security-dashboard");

  // Adiciona animação ao scroll para a seção
...
// Estilos dinamicamente criados para o dashboard
